...
- Import <Term id="DOM" /> scripts (`.webjs`) as strings without the need of an additional bundle;
- Statically check <Term id="DOM" /> scripts for syntax errors;
- Statically check <Term id="DOM" /> scripts compatibility given targeted WebViews versions;
- Test the <Term id="DOM" /> scripts behaviors.
