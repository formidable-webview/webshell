...
the <APIReference reference="handleLinkPressFeature" type="variable" /> implementation.
The implementation is in typescript, which is convenient to communicate the
different types implied in a <Term id="DOM" /> feature. There are four
important areas to specify in such feature:
...
| What type of payload does it ship with events?                                         | `string`           |
...
```ts title="src/features/handle-link-press.ts"
